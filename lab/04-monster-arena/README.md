...
이 실습에서는 여러 몬스터가 하나의 경기장에 입장하여 정해진 규칙에 따라 전투를 수행하는 턴 기반 전투 시뮬레이션인 `Monster Arena`를 구현한다.

경기장은 수용 가능한 최대 인원(capacity)을 가지며 인원이 가득 찬 상태에서는 추가로 몬스터를 입장시킬 수 없다. 경기장에 입장한 몬스터들은 고정된 순서에 따라 매 턴 전투를 수행하며 각 몬스터는 바로 다음에 위치한 몬스터를 공격한다. 마지막 몬스터의 공격 대상은 순환 구조에 따라 경기장의 첫 번째 몬스터가 된다.

전투 도중 몬스터의 체력이 0이 되면 해당 몬스터는 해당 턴의 모든 공격 처리가 끝난 뒤 경기장에서 제거되며 이후 전투에 더 이상 참여하지 않는다. 이 과정을 반복하여 경기장에 하나의 몬스터만 남게 되면 그 몬스터가 최종 생존자가 된다.

이 실습의 목적은 몬스터, 전투 규칙, 속성 상성, 경기장 상태와 같은 요소를 객체 지향적으로 분리하고 모델링하여 여러 클래스가 협력하는 비교적 복잡한 로직을 명확한 구조로 설계하고 구현하는 경험을 하는 데 있다.
